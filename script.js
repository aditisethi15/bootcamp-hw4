...
function initGame() {
    ...
    const symbolPairs = ['🍎', '🍌', '🍇', '🍓', '🍍', '🍉', '🍒', '🥝','🍎', '🍌', '🍇', '🍓', '🍍', '🍉', '🍒', '🥝']; // Create pairs of symbols
    const shuffledSymbols = shuffleArray(symbolPairs); // Shuffle the symbols

    const gameBoard = document.getElementById('game-board');
    gameBoard.innerHTML = ''; // Clear previous cards

    shuffledSymbols.forEach(symbol => {
        console.log("Creating card for symbol:", symbol);
...
function createCard(symbol) {
    ...
    cards[cards.length] = card;
    card.addEventListener('click', () => flipCard(card)); // Add click event
    return card;
}
...
function flipCard(card) {
    ...
    card.textContent = card.dataset.symbol; // Display the symbol

    if (firstCard == null) {
        firstCard = card; // First card picked
    } else {
        secondCard = card; // Second card picked
